...
$(document).ready(function() {
    // Função para abrir modal de arquivos (delegate para elementos dinâmicos)
    $(document).on('click', '.btn-arquivos', function() {
        const id = $(this).data('id');
        const nome = $(this).data('nome');
        
        $('#nomeContratoArquivos').text(nome);
        $('#contrato_id_upload').val(id);
        
        // Carregar lista de arquivos
        carregarArquivos(id, nome);
        
        // Inicializa o modal corretamente com Bootstrap 5
        const arquivosModal = new bootstrap.Modal(document.getElementById('arquivosModal'));
        arquivosModal.show();
    });

    // Enviar formulário de upload via AJAX
    $('#uploadForm').on('submit', function(e) {
        e.preventDefault();
        
        const formData = new FormData(this);
        const contratoNome = $('#nomeContratoArquivos').text();
        
        $.ajax({
            url: 'contratos_upload.php',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            success: function(response) {
                ...
                $('#arquivo').val('');
                carregarArquivos($('#contrato_id_upload').val(), contratoNome);
            },
            error: function() {
...
// Funções auxiliares (fora do document.ready mas no mesmo escopo)
function carregarArquivos(contratoId, contratoNome) {
    $.ajax({
        url: 'contratos_listar_arquivos.php',
        type: 'GET',
        data: { 
            id: contratoId,
            nome: contratoNome
        ...
        success: function(response) {
            $('#listaArquivos').html(response);
        }
    });
}

function visualizarPDF(nomeArquivo, contratoNome) {
    const pastaContrato = '../contratos/' + contratoNome + '/files/';
    $('#pdfViewer').attr('src', pastaContrato + nomeArquivo);
