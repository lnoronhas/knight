                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <td class="text-nowrap">
                                                                        <button class="btn btn-sm btn-info btn-relatorio"
                                                                            data-cliente-id="<?= $cliente['id'] ?>"
                                                                            data-checagem-id="<?= $checagem['id'] ?>">
                                                                            <i class="fas fa-file-pdf"></i> Relatório
                                                                        </button>
                                                                        <?php if ($checagem['caminho_arquivo']): ?>
                                                                            <a href="<?= $checagem['caminho_arquivo'] ?>"
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Configurar comportamento dos radios de frequência
        document.querySelectorAll('input[name="frequencia"]').forEach(radio => {
            radio.addEventListener('change', function () {
                document.getElementById('diaSemanaContainer').classList.add('d-none');
                document.getElementById('diaMesContainer').classList.add('d-none');

                if (this.value === 'semanal') {
                    document.getElementById('diaSemanaContainer').classList.remove('d-none');
                } else if (this.value === 'mensal') {
                    document.getElementById('diaMesContainer').classList.remove('d-none');
                }
            });
        });

        // Configurar botões "Checar Agora"
        document.querySelectorAll('.btn-checar-agora').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                document.getElementById('nomeClienteChecagem').textContent = clienteNome;
                document.getElementById('clienteIdChecagem').value = clienteId;
                confirmarChecagemModal.show();
            });
        });

        // Configurar botão de confirmar checagem
        ...
        if (document.getElementById('cancelarAgendamento')) {
        ...
        // Configurar botão de gerar relatório geral
        ...
        // Configurar botões de relatório individual
        document.querySelectorAll('.btn-relatorio').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                if (checagemId) {
                    // Relatório de uma checagem específica
    ...
    function executarChecagem(clienteId) {
        const btn = document.querySelector(`.btn-checar-agora[data-cliente-id="${clienteId}"]`);
        const originalText = btn.innerHTML;

        // Mostrar indicador de carregamento
        btn.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Checando...';
        btn.disabled = true;
        ...
        fetch('checagem_executar.php', {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert('Erro na checagem: ' + data.message);
                }
            })
            .catch(error => {
                alert('Erro na requisição: ' + error.message);
    ...
    document.querySelectorAll('.btn-editar-agendamento').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            // Carregar dados do agendamento via AJAX e preencher modal
            fetch(`checagem_carregar_agendamento.php?id=${agendamentoId}`)
                .then(response => response.json())
                .then(data => {
                    // Preencher modal com dados
                    // ... implementar conforme estrutura do seu agendamento
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        todosClientes.addEventListener('change', function () {
            if (this.checked) {
                ...
                $('#seletorClientes').val(null).trigger('change');
            } else {
                seletorContainer.style.display = 'block';
            }
        });

        // Desmarca "Todos" quando selecionar algum cliente
    ...
    // Corrige o foco no campo de busca
    $(document).on('select2:open', () => {
        document.querySelector('.select2-search__field').focus();
    });

    // Corrige a cor do texto digitado
    $(document).on('select2:open', () => {
        setTimeout(() => {
